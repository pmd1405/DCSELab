...
def calculate_workspace(num_joints, limit):
    ...
    while initial_angles[0] <= limit:
        ...
        for i in range(num_joints - 1, 0, -1):
            initial_angles[i - 1] = initial_angles[i] + 2
...
# Function to calculate the overlapping area with the workspace
def calculate_overlapping_area(workspace_x, workspace_y):
    # Create a Polygon object from the workspace points
    workspace_polygon = Polygon(list(zip(workspace_x, workspace_y)))
    
    # Calculate the area of the overlapping region
    overlapping_area = workspace_polygon.area
    
    return overlapping_area

# Update function for animation
def update(frame, joint_positions, ax, workspace_x, workspace_y, workspace_area, overlapping_area):
    ax.clear()
    ...
    ax.set_title(f'Frame {frame}, Workspace Area: {workspace_area:.2f} mm^2, Overlapping Area: {overlapping_area:.2f} mm^2')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.grid(True)
    ax.axis('equal')
...
def main():
    ...
    for idx, num_joints in enumerate(num_joints_list):
        ...
        overlapping_area = calculate_overlapping_area(workspace_x, workspace_y)
        ax = axs[idx // cols, idx % cols] if num_plots > 1 else axs
        anim = FuncAnimation(fig, update, frames=len(joint_positions), fargs=(joint_positions, ax, workspace_x, workspace_y, workspace_area, overlapping_area), interval=50, repeat=False)
